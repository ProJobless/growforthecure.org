<div class="inner">
		
	<div class="subnav">
	<a href="<?php echo base_url() ?>about/">About</a>
	<a href="<?php echo base_url() ?>about/fundraising/">Fundraising Tips</a>
	<a href="<?php echo base_url() ?>about/faq/">Frequently asked questions</a>
	<a href="<?php echo base_url() ?>about/lung_cancer_facts/">Lung Cancer Facts</a>
	<a href="<?php echo base_url() ?>about/beards101/">Beards 101</a>
	</div>

		<img src="<?php echo base_url() ?>artwork/frequentlyaskedquestions.png" />

		<p class="faq-header">General</p>

		<p class="question">What is "Grow for the cure"?</p>
		<p class="answer">Grow for the Cure is a fundraising website dedicated to increasing the money available for lung cancer research. We are an affiliate of the non-profit Bonnie J. Addario Lung Cancer Foundation and operate under their 501(c)3 status. We have a platform where volunteers can register to grow facial hair. People can select from a grid of facial hair types the volunteer will grow and donate money to their favorite style.</p>

		<p class="question">What is Bonnie J. Addario Lung Cancer Foundation?</p>
		<p class="answer">The Lung Cancer Foundation (LCF) is one of the largest patient-driven philanthropies nationwide devoted exclusively to eradicating Lung Cancer through research, awareness, education, early detection and treatment. The Foundation works with diverse partners (physicians, organizations, industry partners, thought leaders, survivors and their families) in taking actions NOW in the laboratory, in hospitals and in the public forum that lead to solutions for patients. Since its founding as a 501(c) 3 non-profit organization in CA in 2006, the LCF has raised more than $10 million dollars for Lung Cancer research, patient services and awareness.</p>

		<p class="question">How is the money raised, used?</p>
		<p class="answer">All net proceeds are used by the Bonnie J. Addario Lung Cancer Foundation. Their top physicians and researchers select from immediate results-oriented projects that promise to catalyze progress through early detection, genetic testing, drug discovery and patient-focused outcomes.</p>

		<p class="question">Why did you partner with them?</p>
		<p class="answer">We realized the need for a larger partner that has the resources to make the most of your donations.</p>

		<p class="question">Is my donation tax deductible?</p>
		<p class="answer">Yes, as a qualified non-profit affiliate, all donations are tax deductible. Please visit <a href="http://www.lungcancerfoundation.org/contribute/">http://www.lungcancerfoundation.org/contribute/</a> for more information.</p>

		<p class="question">What is your Federal Tax ID number?</p>
		<p class="answer">Employee Identification Number: 20-4417327 DLN: 17053090042016 Effective Date of Exemption: March 1, 2006</p>

		<p class="question">How can I get involved?</p>
		<p class="answer">Please visit the BJALCF website or any of the links under the Resources tab for more information on what you can do to make an impact on lung cancer.</p>

		<p class="faq-header">Registration</p>

		<p class="question">How do I register?</p>
		<p class="answer">Click on Become a Grower and choose whether you’d like to register as an individual, join a team or start a team. Some general information will be needed for registration - complete all fields with an asterisk.</p>

		<p class="question">I forgot my password.</p>
		<p class="answer">Under the login function, there is an option for ‘forgot my password?’ Simply enter your email address and click ‘submit’. You will receive an automated email from Grow for the Cure, which will allow you to reset your password. If you don’t receive this email, please contact us at <a href="mailto:webmaster@growforthecure.org">webmaster@growforthecure.org</a> and we can investigate further.</p>

		<p class="question">Can I close and delete my profile page?</p>
		<p class="answer">Yes, simply email <a href="mailto:webmaster@growforthecure.org">webmaster@growforthecure.org</a> with your user name and it will be taken down after the duration you chose for the project has expired.</p>

		<p class="question">What if I already have a beard?</p>
		<p class="answer">That’s great! You are a step ahead. Proceed with registration and invite your friends to decide what style you will have to trim.</p>

		<p class="faq-header">Donations</p>


		<p class="question">How do I donate?</p>
		<p class="answer">You can donate the following ways:
			<ul class="donate-list">
				<li><strong>Online using PayPal</strong></li>
				<li><strong>Over the phone by calling 650.598.2857</strong></li><br />
				<li>Send in a check or money order along with the mail in donation form, found under the Shop/Give tab to: <br />
					<strong>Grow for the Cure<br />
					C/o Bonnie J. Addario Lung Cancer Foundation<br />
					1100 Industrial Road Suite 1<br />
					San Carlos, CA 94070</strong></li>
			</ul>

		<p class="question">Is you online donation system secure?</p>
		<p class="answer">Yes, it is secured and operated by PayPal.</p>

		<p class="question">Do I get a tax receipt for my donation?</p>
		<p class="answer">After you make your donation, you will be directed to a thank you page that you can print as your receipt. It will include your donation amount and the BJALCF tax ID number.</p>

<br clear="all" />
<br clear="all" />
<br clear="all" />


	</div>

</div>